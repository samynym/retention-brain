...
This document explains *why* `rc-retention-brain` is shaped the way it is. The code shows what it does; this document shows the mental model behind the decisions. Read this first if you want to understand the project's center of gravity in five minutes.

## TL;DR

Four framework moves drive everything below:

1. **Operator, not analyst.** Existing subscription tooling tells you what happened (Rico, Mixpanel, RC dashboard) or lets you build flows (Customer.io, Braze). This brain *decides what to do per user, in writing, ready to send*. v1.0 stops at "draft the action"; v1.x graduates to executing it.
...
**Analyst-tier products** (Rico, Mixpanel, every "AI assistant for X dashboard") tell you what happened and recommend what to do. The user — a human — still has to translate insight into action: open a different tool, write the email, decide who to send to, pick a discount, hit send.
...
The gap between these is wider than it looks. Rico is RevenueCat's own AI agent, embedded in the dashboard and Slack. Its positioning page (revenuecat.com/feature/ai-agent) is explicit:

> "An advisor, not a search bar."

That's analyst-tier by design. The brain fills the operator-tier gap — but it doesn't claim to be the operator on day one. It claims to be **the operator's analyst on day one**, with the operator landing in v1.1.
...
Most retention tools commit you to a fixed shape upfront. Customer.io is "all sources up front, flow-builder UI, autonomous from setup." Rico is "RC-only, recommend-only." The brain runs at every corner of this grid:

```
                  Sources wired
              1 key ────────────► 4 keys (and beyond, via MCP)
              RC only             RC + Stripe + Sentry + PostHog + ...
              (minimum)           (cathedral)
Trust level
Level 0 ──┐   Briefing on RC      Briefing across all signals
brief     │   alone, ~2 min        — usage decline, error rate,
only      │   install              payment health, support tone
          │
Level 1   │   Approve-each via    Approve-each across all channels
          │   email on RC events
          │
Level 5   │   Autonomous on       Autonomous reactive operator
auto      │   RC-only retention   across the whole signal surface
real-time │
          ▼
```
...
- **A first-time installer** pastes one RC key, sees a briefing in two minutes, and stays at Level 0 indefinitely. Zero commitment.
- **A motivated indie dev** adds Stripe and PostHog over the next week as they read more signal types in the briefing.
...
- **Zero per-vendor maintenance.** When RC ships a new field or Stripe deprecates an endpoint, the upstream MCP server handles it. The brain doesn't care.
- **Source 5+ is a JSON config entry.** Wiring a new source = add an object to `.rcrb/mcp.json` with `command`/`url`, `tool`, `mapper`, and an optional `hint`. No code.
...
- Two of the official MCPs (RevenueCat, Stripe) don't yet expose batch event-by-date-range tools — only per-customer queries. The brain plugs the gap with its own `webhook-receiver` package: a small HTTP server that catches RC + Stripe webhooks, stores them in `.rcrb/events.jsonl`, and fronts that file as an MCP server (`rcrb events-mcp-server`). It's MCP-shaped from the brain's perspective; the file is just the persistence layer.
...
### What you wire in `.rcrb/mcp.json`
...
The renderer mentions these tickets at the top of the briefing ("3 stabilization tickets in `./engineering-tickets/`") and links to them from the relevant per-user blocks. A future workflow (e.g., `rcrb sync-tickets --target=linear`) can map them onto whatever issue tracker is in play. v1.0 stops at the Markdown layer.
...
## 6. Webhook receiver: filling the gap RC and Stripe don't
...
- `rcrb webhook-listen` starts a small Node HTTP server (Stripe HMAC signature verification + RC shared-secret comparison, both using `timingSafeEqual`)
- Catches POSTs to `/webhooks/stripe` and `/webhooks/revenuecat`
- Maps them into the brain's normalized `Event` schema and appends to `.rcrb/events.jsonl`
- `rcrb events-mcp-server` fronts the same file as a stdio MCP — wired into `.rcrb/mcp.json` like any other source
...
- **RevenueCat Promotional Offers channel.** The most-native retention play for an RC merchant — issue a StoreKit / Play Billing offer directly via the RC API; appears native in-app, no email/push. Defensible against Rico because it's outcome-as-action, not analysis. Lands in v1.1 alongside email + push (v1.1 ships multi-channel from day one of executable interventions).
...
| v1.1 | Level 1 (approve-each) | Resend email · push (OneSignal / FCM) · RC Promotional Offers (native in-app via StoreKit / Play Billing) | First operator capabilities land — multi-channel from day one of executable interventions. Outcomes ledger starts. Optional `rcrb sync-tickets` adapter for the engineering-tickets folder. |
...
**Source coverage is not version-tagged.** New sources (Mixpanel, Amplitude, Crisp, Intercom, Firebase, App Store reviews — anything with an MCP server) slot in via `.rcrb/mcp.json` as the user wants them, independently of the trust-and-execution roadmap above. The MCP-only architecture is precisely what makes this possible.
...
- `intervention.ts` — the `Intervention` schema. What the agent produces; what `rcrb approve` operates on in v1.1; what the briefing renders.
